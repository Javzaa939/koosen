...
const UsgenUnelgee = () => {
	...
	async function getDatas() {
		const { success, data } = await fetchData(gradeletterApi.get())
		if(success) {
			setDatas(data)
	...
	async function handleDelete(id){
		if(id){
			const{ success} = await fetchData(gradeletterApi.delete(id))
			if (success){
				getDatas()
			}
		}
	...
	const handleFilter = (e) => {
		...
		if (value.length) {
			updatedData = datas.filter((item) => {
				const startsWith =
					item.letter.toString().toLowerCase().startsWith(value.toString().toLowerCase()) ||
					item.description.toString().toLowerCase().startsWith(value.toString().toLowerCase())

				const includes =
					item.letter.toString().toLowerCase().includes(value.toString().toLowerCase()) ||
					item.description.toString().toLowerCase().includes(value.toString().toLowerCase())
		...
		<Fragment>
			<Card>
				<CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
					<CardTitle tag="h4">{t('Үсгэн үнэлгээ')}</CardTitle>
                    <div className='d-flex flex-wrap mt-md-0 mt-1'>
                        <Button
							color='primary'
							onClick={() => handleModal()}
							disabled={Object.keys(user).length > 0 && user.permissions.includes('lms-settings-season-create') ? false : true}
						>
                            <Plus size={15} />
