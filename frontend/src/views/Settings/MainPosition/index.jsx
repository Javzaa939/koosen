...
const MainPosition = () => {
	...
	async function getDatas() {
		const { success, data } = await fetchData(mainpositionApi.get())
		if(success) {
			setDatas(data)
	...
	async function handleDelete(id){
		if(id){
			const{ success} = await fetchData(mainpositionApi.delete(id))
			if (success){
				getDatas()
			}
		}
		...
		<Fragment>
			<Card>
				<CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
					<CardTitle tag="h4">{t('Үндсэн албан тушаалын төрөл')}</CardTitle>
