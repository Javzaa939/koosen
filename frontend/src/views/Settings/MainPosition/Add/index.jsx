...
const Addmodal = ({ open, handleModal, refreshDatas ,editId}) => {
	...
	// Api
	const mainPositionApi = useApi().settings.mainposition

	async function onSubmit(cdata) {
         cdata = convertDefaultValue(cdata)
        if(editId) {
            const { success, errors } = await fetchData(mainPositionApi.put(cdata, editId))
            if(success) {
                reset()
                refreshDatas()
                handleModal()
            ...
            const { success, error } = await postFetch(mainPositionApi.post(cdata))
            if(success) {
                reset()
                handleModal()
                refreshDatas()
    ...
    async function getDatas() {
        if(editId) {
            const { success, data } = await fetchData(mainPositionApi.getOne(editId))
            if(success) {
        ...
        <Fragment>
            ...
            >
                <ModalHeader
                    ...
                    tag="div"
                >
                    <h5 className="modal-title">{ editId ? t('Үндсэн албан тушаалын төрөл засах') : t('Үндсэн албан тушаалын төрөл нэмэх')}</h5>
                </ModalHeader>
