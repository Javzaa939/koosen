	...
	const handleFilter = (e) => {
		...
		if (value.length) {
			updatedData = datas.filter((item) => {
				const startsWith =
					item.season_code.toString().toLowerCase().startsWith(value.toString().toLowerCase()) ||
					item.season_name.toString().toLowerCase().startsWith(value.toString().toLowerCase())

				const includes =
					item.season_code.toString().toLowerCase().includes(value.toString().toLowerCase()) ||
					item.season_name.toString().toLowerCase().includes(value.toString().toLowerCase())
		...
		<Fragment>
			<Card>
				<CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
					<CardTitle tag="h4">{t('Улирал')}</CardTitle>
