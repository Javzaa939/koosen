	...
	const handleFilter = (e) => {
		...
		if (value.length) {
			updatedData = datas.filter((item) => {
				const startsWith =
					item.category_code.toString().toLowerCase().startsWith(value.toString().toLowerCase()) ||
					item.category_name.toString().toLowerCase().startsWith(value.toString().toLowerCase())

				const includes =
					item.category_code.toString().toLowerCase().includes(value.toString().toLowerCase()) ||
					item.category_name.toString().toLowerCase().includes(value.toString().toLowerCase())
		...
		<Fragment>
			<Card>
				<CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
					<CardTitle tag="h4">{t('Хичээлийн ангилал')}</CardTitle>
                    <div className='d-flex flex-wrap mt-md-0 mt-1'>
                        <Button color='primary'
							disabled={Object.keys(user).length > 0 && user.permissions.includes('lms-settings-lessoncategory-create') ? false : true}
							onClick={() => handleModal()}>
                            <Plus size={15} />
