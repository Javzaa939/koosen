...
const Score = () => {
	...
	async function getDatas() {
		const { success, data } = await fetchData(scoreApi.get())
		if(success) {
			setDatas(data)
	...
	const handleFilter = (e) => {
		...
		if (value.length) {
			updatedData = datas.filter((item) => {
				const startsWith =
					item.score_code.toString().toLowerCase().startsWith(value.toString().toLowerCase()) ||
					item.score_max.toString().toLowerCase().startsWith(value.toString().toLowerCase()) ||
					item.score_min.toString().toLowerCase().startsWith(value.toString().toLowerCase()) ||
					item.assesment.toString().toLowerCase().startsWith(value.toString().toLowerCase())

				const includes =
					item.score_code.toString().toLowerCase().includes(value.toString().toLowerCase()) ||
					item.score_max.toString().toLowerCase().includes(value.toString().toLowerCase()) ||
                    item.score_min.toString().toLowerCase().includes(value.toString().toLowerCase()) ||
					item.assesment.toString().toLowerCase().includes(value.toString().toLowerCase())
		...
		<Fragment>
			<Card>
				<CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
					<CardTitle tag="h4">{t('Үнэлгээний бүртгэл')}</CardTitle>
                    <div className='d-flex flex-wrap mt-md-0 mt-1'>
                        <Button
							color='primary'
							onClick={() => handleModal()}
							disabled={Object.keys(user).length > 0 && user.permissions.includes('lms-settings-score-create') ? false : true}
						>
                            <Plus size={15} />
