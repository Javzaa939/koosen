...
const Country = () => {
	...
	async function handleDelete(id){
		if(id){
			const{ success} = await fetchData(countryApi.delete(id))
			if (success){
				getDatas()
			}
		}
		...
		if (value.length) {
			filterData = datas.filter((item) => {
				...
				const includes =
					...
					item.name_eng.toString().toLowerCase().includes(value.toString().toLowerCase())
		...
		<Fragment>
			<Card>
				<CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
					<CardTitle tag="h4">{t('Улсын мэдээлэл')}</CardTitle>
                    <div className='d-flex flex-wrap mt-md-0 mt-1'>
                        <Button
							color='primary'
							onClick={() => handleModal()}
							disabled={Object.keys(user).length > 0 && user.permissions.includes('lms-settings-country-create') ? false : true}
						>
                            <Plus size={15} />
