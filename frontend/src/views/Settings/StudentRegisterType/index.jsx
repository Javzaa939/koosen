...
const StudentRegisterType = () => {
	...
	async function getDatas() {
		const { success, data } = await fetchData(registerType.get())
		if(success) {
			setDatas(data)
		...
		<Fragment>
			<Card>
				<CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
					<CardTitle tag="h4">{t('Оюутны бүртгэлийн хэлбэр')}</CardTitle>
                    <div className='d-flex flex-wrap mt-md-0 mt-1'>
                        <Button
							color='primary'
							onClick={() => handleModal()}
							disabled={Object.keys(user).length > 0 && user.permissions.includes('lms-settings-registerstatus-create') ? false : true}
						>
                            <Plus size={15} />
...
export default StudentRegisterType;
