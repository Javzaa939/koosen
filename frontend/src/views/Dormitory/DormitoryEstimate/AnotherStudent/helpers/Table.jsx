...
const CTable = ({ datas, currentPage, rowsPerPage, page_count, handlePagination, CSum, handleSort, refreshDatas }) => {
    ...
    // Api
    const dormitoryEstimateAnotherApi = useApi().dormitory.estimate.another
    ...
    async function handleChange(edit_id) {
        ...
        if(!isObjEmpty(cdatas) && edit_id) {
            const { success } = await fetchData(dormitoryEstimateAnotherApi.put(cdatas, edit_id))
            if(success) {
                refreshDatas()
            }
        }
                            ...
                            datas.map((row, idx) => {
                                return (
                                    <tr key={idx}>
                                        ...
                                        <td className='bg-transparent'>{row?.student?.full_name}</td>
                                        <td className='bg-transparent'>{row?.student?.register}</td>
