...
import { get_leveltype, get_questionype, ReactSelectStyles } from "@utils";
...
export default function EditModal({ open, handleModal, questionDetail, getDatas }) {
    ...
    async function handleQuestionSave(questionId) {
        ...
        const { success, data } = await fetchData(questionAPI.putTestQuestions(formData, questionId));
        if (success) {
            if (data) {
    ...
    async function handleAnswerSave(questionId, answerId) {
        ...
        const { success, data: newData } = await fetchData(questionAPI.putTestQuestions(formData, questionId, "answer"));
        if (success) {
            if (newData) {
