...
// Хүснэгтийн баганууд
export function getColumns (currentPage, rowsPerPage, datas, handleEditModal, handleSolvedModal, detailModal, roleMenus, handleApproveModal) {
	...
	function request_answer(row, unit) {
		...
		if (f_answer?.is_answer) {
			...
			return (
				<>
					<Square width={"15px"} className='cursor-pointer relative' onClick={() => handleSolvedModal(row?.id, row?.profession, unit)} />
					<span className='text-danger'>*</span>
				</>
			)
		}
	...
	const columns = [
		...
		{
			header: 'correspond_type',
			name: `${t('Дүйцүүлэх хүсэлтийн төрөл')}`,
            cell: (row) => row?.correspond_type_name,
			minWidth: "80px",
			sortable: true,
			center: true
		},
						...
						:
							...
							?
								<Button
									color='primary'
									size='sm'
									onClick={() => { row?.isSolved === 2 ? handleSolvedModal(row?.id, row?.profession, COMPLAINT_UNIT_BMA_ID) : handleApproveModal(row?.id) } }
								>
					...
					<div className="text-center ms-0" style={{ width: "auto" }}>
						{
							...
							<>
								<a role="button" onClick={() => { detailModal(row?.id, row?.profession) } }
						...
						{
							row?.is_solved !== COMPLAINT_ALLOW && row?.isSolved === 0 && user.permissions.includes('lms-request-correspond-update') &&
							<>
								<a role="button" onClick={() => { handleEditModal(row.id)} }
									id={`correspondDatatableEdit${row?.id}`}
									className='me-1'
								>
									<Badge color="light-secondary" pill><Edit width={"15px"}/></Badge>
								</a>

								<UncontrolledTooltip placement='top' target={`correspondDatatableEdit${row.id}`} >Засах</UncontrolledTooltip>
							</>
						}
						{
                            row?.is_solved == 3
                            ...
                            <>
                                <Link
                                    ...
                                    to={`/request/correspond/print/${row.id}`}
                                    target='_blank'
                                    className="ms-1"
                                >
