...
const Addmodal = ({ open, handleModal, refreshDatas, editValues }) => {
    var values = {
        lesson: '',
        teacher: '',
        potok: '',
    ...
    function getAll() {
        Promise.all(
            [
                fetchData(fetchData(roomApi.getList())),
                fetchData(fetchData(lessonApi.getTimetableList())),
            ]
        ).then((values) => {
            setRoom(values[0]?.data)
            setLessonOption(values[1]?.data)
        })
    }
    ...
    function getStudents() {
        ...
        Promise.all(
            [
                fetchData(studentApi.getGroup(group_ids, true)),
                fetchData(studentApi.getRemoveGroup(group_ids, false))
            ]
        ).then((values) => {
    ...
    useUpdateEffect(
        () =>
        {
           if (select_value?.lesson) getTeacher()
        },
        [select_value?.lesson]
    )
	...
	async function onSubmit(cdata) {
        ...
        if (!checked && selectedGroups.length == 0) {
            ...
            const { success, error } = await fetchData(timetableApi.post(cdata))
            if(success) {
                reset()
                handleModal()
                refreshDatas()
