    ...
    return (
        <Col>
            ...
            <Select
                ...
                value={
                    select_value &&
                    groupOption.find((c) => c.id === select_value)
                }
...
const GradeList = () => {
        ...
        },
        {
            ...
            component: (
                <Student
                    setMainData={setMainData}
                    setChosenGroup={setChosenGroup}
                    chosenGroupStudent={chosenGroupStudent}
                    setChosenGroupStudent={setChosenGroupStudent}
                    setFileName={setGroupName}
                    setStudentSectionData={setStudentSectionData}
                />
            ),
        },
    ];
    ...
    async function getGroupLesson() {
        const { success, data } = await fetchData(
            lessonApi.getLessonsGroup(chosenGroup)
        );
        if (success) {
    ...
    async function onSubmit() {
        if (file) {
            ...
            const { success, data } = await fetchData(
                scoreApi.postOldScore(formData)
            );
            if (success) {
                handleFileModal();
    ...
    async function onSubmitV2() {
        if (fileV2) {
            ...
            const { success, data } = await fetchData(
                scoreApi.postOldScoreV2(formData)
            );
            if (success) {
                handleFileModalV2();
                handleShowDetailModal();
    ...
    function navigatePrint() {
        navigate(`/score/preview-score/print`, {
            state: { mainData, chosenGroup },
        });
    }
    ...
    function excelAngi() {
        ...
        const staticCells = ["№", "Оюутны код", "Оюутны нэр", ...lessons];
        ...
        const columns = studentOption.map((col, idx) => {
            ...
            lessons.forEach((lesson) => (dynamicKey[lesson] = ""));
        ...
        worksheet["!cols"] = [
            { wch: studentOption.length > 100 ? 3 : 2 },
            { wch: 15 },
            { wch: 20 },
            { wch: 20 },
            { wch: 11 },
        ...
        <Fragment>
            ...
            {open_filev2 && (
                <FileModal
                    ...
                    disabled={
                        !select_value || Object.keys(select_value).length === 0
                    }
                />
            )}
