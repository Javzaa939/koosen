...
export function getColumns(
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row) => {
                return row?.duration ? <>{row?.duration}&nbsp;минут</> : <></>;
            },
            sortable: true,
            maxWidth: '200px',
            minWidth: '100px',
            ...
            selector: (row) => (
                <div>
                    <span>
                        ...
                        <UncontrolledTooltip
                            placement="top"
                            target={`requestVolunteerDetail${row.id}`}
                        >
                            Ороцогчид
                        </UncontrolledTooltip>
                    </span>
                    {row.test_type === TYPE_IQ && (
                        <span>
                            <a
                                ...
                                className="me-1"
                                onClick={() => {
                                    IQresultExcelReport(active, excelApi, adm, row, fetchData);
                                }}
                            ...
                            <UncontrolledTooltip placement="top" target={`iqTestTooltip${row.id}`}>
                    ...
                    {row.test_type === TYPE_PSY && (
                        <span>
                            <a
                                ...
                                className="me-1"
                                onClick={() => {
                                    resultExcelReport(active, excelApi, adm, row, fetchData);
                                }}
                                disabled={!(adm || active !== 2)}
                            >
                                <Badge color="primary" pill>
                            ...
                            <UncontrolledTooltip placement="top" target={`psyTestTooltip${row.id}`}>
