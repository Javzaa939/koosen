...
const SciencePaper = () => {
    ...
    const [published_yearOption, setPublished_yearOption] = useState(generateYear(3))
    ...
    const categoryApi = useApi().science.paper.category
    ...
    async function getDatas() {
        ...
        const { success, data } = await allFetch(scienceApi.paper.get(rowsPerPage, currentPage, sortField, searchValue, department, teacher, published_year, category))
        if(success) {
    ...
    async function getCategoryOption() {
        const { success, data } = await fetchData(categoryApi.getList())
        if(success) {
        ...
        <Fragment>
            <Card>
                ...
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>{t('Эрдэм шинжилгээ өгүүлэл жагсаалт')}</CardTitle>
                </CardHeader>
                            ...
                            render={({ field: { value, onChange} }) => {
                                return (
                                    <Select
                                        ...
                                        value={published_yearOption.find((item) => item.id === select_value.category)}
                                        noOptionsMessage={() => t('Хоосон байна.')}
                                        onChange={(val) => {
                                            onChange(val?.id || '')
                                            setSelectValue(current => {
