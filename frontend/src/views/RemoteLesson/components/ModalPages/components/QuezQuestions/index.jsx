...
import QuezChoices from '../QuezChoices';


export default function QuezQuestions({
	t,
	control,
	errors,
	isLoading,
	...
	// #region items
	const [onlineInfos, setOnlineInfos] = useState(getValues(`${parentName}.quezQuestions`)?.length || 0);
	...
	const kindOptions = [
		{ id: 1, name: 'Нэг сонголт' },
		{ id: 2, name: 'Олон сонголт' },
		{ id: 3, name: 'Үнэн, Худлыг олох' },
		{ id: 4, name: 'Үнэлгээ' },
		{ id: 5, name: 'Эссэ бичих' },
		{ id: 6, name: 'Богино нөхөх хариулт' },
		{ id: 7, name: 'Харгалзуулах, жиших' },
		{ id: 8, name: 'Тооцоолж бодох' },
		{ id: 9, name: 'Төсөл боловсруулах' },
		{ id: 10, name: 'Товч хариулт' },
	]
				...
				<Accordion open={open} toggle={toggle}>{/*console.log(onlineInfos)*/}
					{
						new Array(onlineInfos).fill(null).map((onlineInfos_item, onlineInfos_ind) => {
							const inputNameElement = `${parentName}.quezQuestions.${onlineInfos_ind}`
							const inputName = `${inputNameElement}.question`
							const inputNameKind = `${inputNameElement}.kind`
							...
							return <AccordionItem key={onlineInfos_ind}>
								...
								<AccordionBody accordionId={`accordionId_${onlineInfos_ind}`}>
									<Row>
										<Col md={6}>
											<Label className="form-label" for={inputNameKind}>
												{t('Асуултын төрөл')}
											</Label>
											<Controller
												control={control}
											...
											></Controller>
											{errors[inputNameKind] && <FormFeedback className='d-block'>{t(errors[inputNameKind].message)}</FormFeedback>}
										</Col>
									</Row>
									<QuezChoices
										t={t}
