...
export const validateSchema = Yup.object().shape({
    description: Yup.string()
        .trim()
        .required('M0 оруулна уу.')
        .max(160, '160-иас бага тэмдэгт оруулна')
});
...
function MessageModal({ messageModalHandler, messageModal, selectedStudents, getDatas }) {
    ...
    async function onSubmit(cdata) {
        ...
        cdata['phone_numbers'] = selectedStudents.map(val => val?.user?.mobile) || [];
        ...
        >
            ...
            <ModalBody className='d-flex flex-column justify-content-between' style={{ minHeight: 300 }} tag={Form} onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <Col lg={6} md={12}>
                            <div className='rounded-3 mx-25'>
                                <Label className='p-50' htmlFor='description'>
                                    Илгээж мессежийн утга (160 тэмдэгтээс ихгүй утга)
                                </Label>
                                <Controller
                                    control={control}
                                    defaultValue=''
                ...
                <div className='text-center my-50'>
                    ...
                    <Button className='m-50 ' onClick={() => messageModalHandler()} disabled={isLoading}>Цуцлах</Button>
                </div>
            </ModalBody>
        </Modal>
    )
