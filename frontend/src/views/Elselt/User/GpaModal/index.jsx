...
function GpaModal({
	...
	async function saveData(cdata) {
		if (cdata) {
			const { success, data } = await fetchData(saveApi.get(cdata.count, lesson_year, prof_id, cdata.gpa))
			if (success) {
				reset();
			}
		}
								...
								<Controller
									...
									render={({ field: { value, onChange } }) => (
										<Input
											...
											id="gpa"
											invalid={errors?.gpa}
											placeholder='Голч оноо оруулна уу'
											value={value}
								...
								<Controller
									...
									render={({ field: { value, onChange } }) => (
										<Input
											...
											name="count"
											id="count"
											placeholder="Тэнцүүлэх элсэгчдийн тоо"
						...
						<Button
							...
							onClick={() =>
								showWarning({
									...
									question: `Хадгалснаар тэнцээгүй элсэгчдийн элсэлт төлөв тэнцээгүй болж өөрчлөгдөхийг анхаарна уу?`,
