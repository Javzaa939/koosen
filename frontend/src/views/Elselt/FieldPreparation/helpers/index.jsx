    ...
    const columns = [
		{
			...
			selector: (row, index) => {
				...
				{
					let border =
						row?.army_data?.state == 1 ? ''
						: row?.army_data?.state == 2 ? '4px solid rgba(40, 199, 111, 1)'
						: row?.army_data?.state== 3 ? '4px solid #EA5455' : '4px solid transparent'
		...
		},
		{
			...
			selector: (row) => (
				<Badge
					color={`${row?.army_data?.state == 1 ? 'warning	' : row?.army_data?.state == 2 ? 'success' : row?.army_data?.state == 3 ? 'danger' : 'primary'}`}
					pill
				>
					{row?.army_data ?
						stateop	.find(val => val.id === row?.army_data?.state).name
						:
						''
						}
				</Badge>),
...

		{
			...
			header: 'description',
			name: t("Тайлбар"),
			cell: (row) => (row?.army_data?.description),
			sortable: true,
			reorder: true,
			center: true,
			wrap: true,
