...
const LessonStandart = () => {
    ...
    async function postExcel(data) {
        const formData = new FormData();
        formData.append('file', data);
        const { success } = await fetchData(lessonStandartApi.postExcel(formData));
    }
        ...
        <Fragment>
                    ...
                    <div className="d-flex flex-wrap mt-md-0 mt-1 gap-1">
                        ...
                        <Button
                            ...
                            disabled={
                                ...
                                user.permissions.includes('lms-study-lessonstandart-create') &&
                                school_id
                                    ? false
                                    : true
                            }
                            ...
                            render={({ field: { value, onChange } }) => {
                                return (
                                    <Select
                                        ...
                                        isClearable
                                        className={classnames('react-select', {
                                            'is-invalid': errors.lessonCategory,
                                        })}
                                        isLoading={isLoading}
                                        placeholder={t('-- Сонгоно уу --')}
            ...
            {isEditScore && (
                <GroupEditScore
                    open={isEditScore}
                    handleModal={() => {
                        setIsEditScore(!isEditScore), setRowData({});
                    }}
                    dep_id={dep_id}
                    rowData={rowData}
                />
