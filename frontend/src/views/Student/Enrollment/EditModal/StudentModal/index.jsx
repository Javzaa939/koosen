...
function StudentList({ open, handleModal, handleSelectedModal, datas }) {
        ...
        if (value.length) {
            updatedData = datas.filter((item) => {
                const startsWith =
                    ...
                    item?.register?.toLowerCase().startsWith(value.toLowerCase())

                const includes =
                    item?.first_name?.toLowerCase().includes(value.toLowerCase()) ||
                    item?.last_name?.toLowerCase().includes(value.toLowerCase()) ||
                    item?.register?.toLowerCase().includes(value.toLowerCase())
        ...
        <Fragment>
			...
			>
                ...
                <Col md={12} className="mb-2 text-center">
                    ...
                    <Button color="secondary" size='sm' type="reset" outline  onClick={() => handleModal(!open)}>
                        {t('Буцах')}
                    </Button>
                </Col>
            </Modal>
