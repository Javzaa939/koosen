...
const Health = () => {
    ...
    async function getDatas() {
        ...
        const { success, data } = await allFetch(healthApi.get(rowsPerPage, currentPage, sortField, searchValue))
        if (success) {
	...
	const handleDelete = async(id) => {
        const { success } = await fetchData(healthApi.delete(id))
        if(success)
        {
            getDatas()
        }
        ...
        <Fragment>
            ...
            {isLoading && Loader}
                <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
                    <CardTitle tag="h4">{t('Эрүүл мэндийн танилцуулга')}</CardTitle>
                    <div className='d-flex flex-wrap mt-md-0 mt-1'>
                        <Button
                            color='primary'
                            onClick={() => handleModal()}
                            disabled={Object.keys(user).length > 0 && user.permissions.includes('lms-browser-health-create') ? false : true}
                        >
                            <Plus size={15} />
