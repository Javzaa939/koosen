...
const Structure = () => {
    ...
    async function getDatas() {
        ...
        const { success, data } = await allFetch(browserApi.get(rowsPerPage, currentPage, sortField, searchValue))
        if (success) {
        ...
        <Fragment>
            ...
            {isLoading && Loader}
                <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
                    <CardTitle tag="h4">{t('Их сургуулийн бүтэц зохион байгуулалт')}</CardTitle>
                    <div className='d-flex flex-wrap mt-md-0 mt-1'>
                        <Button
                            color='primary'
                            onClick={() => handleModal()}
                            disabled={Object.keys(user).length > 0 && user.permissions.includes('lms-browser-structure-create') ? false : true}
                        >
                            <Plus size={15} />
