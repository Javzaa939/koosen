...
const AnotherStudent = () => {
    ...
    // Api
    const dormitoryReqApi = useApi().dormitory.request.another

	    async function getDatas() {
        const { success, data } = await allFetch(dormitoryReqApi.get(rowsPerPage, currentPage, sortField, searchValue))
        if(success) {
    ...
    async function handleViewModal(id, data) {
        // setViewModalOpen(!view_modal_open)
        // setModalOpen(!modal_open)
        setReqId(id)
        setView(true)
            ...
            {Loader && isLoading}
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom pt-0'>
                    <CardTitle tag="h4" className='d-flex justify-content-between w-100'>{t('Дотуур байранд амьдрах хүсэлт')}
                        ...
                        <AddModal toggle={toggle} modal={amodal} handleModal={toggle} refreshDatas={getDatas} />
                    </CardTitle>
                </CardHeader>
