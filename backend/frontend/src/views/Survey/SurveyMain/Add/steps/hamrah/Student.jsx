					...
					({
						...
						({
							...
							({
								name: citem3.name,
								id: `salbar${citem3?.id}`,
								children: citem3.children ? citem3.children.map((citem4) =>
								({
									name: citem4.name,
									id: `pro${citem4?.id}`,
									children: citem4.children ? citem4.children.map((citem5) =>
									({
										id: citem5?.id,
										name: citem5.name,
									})) : []
					...
					<div className={classnames('cardMaster rounded border p-1')}>
						...
						<div className="mb-1">
							<Input
								...
								name={'all'}
								checked={isAll}
								onChange={(e) => {setIsAll(e.target.checked), onChosenScopeChange({'isAllStudent': true})}}
							/>
