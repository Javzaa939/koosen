	...
	const handleFilter = (e) => {
		...
		if (value.length) {
			updatedData = datas.filter((item) => {
				const startsWith =
					item.group_code.toString().toLowerCase().startsWith(value.toString().toLowerCase()) ||
					item.group_name.toString().toLowerCase().startsWith(value.toString().toLowerCase())

				const includes =
					item.group_code.toString().toLowerCase().includes(value.toString().toLowerCase()) ||
					item.group_name.toString().toLowerCase().includes(value.toString().toLowerCase())
				...
				<CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
                    ...
                    <div className='d-flex flex-wrap mt-md-0 mt-1'>
                        <Button
							color='primary'
							onClick={() => handleModal()}
							disabled={Object.keys(user).length > 0 && user.permissions.includes('lms-settings-lessongroup-create') ? false : true}
						>
                            <Plus size={15} />
