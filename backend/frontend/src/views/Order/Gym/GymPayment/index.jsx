...
const GymPayment = () => {
    ...
    async function getDatas() {
        const { success, data } = await allFetch(gymPaymentApi.get(rowsPerPage, currentPage, sortField, searchValue))
        if(success) {
    ...
    // Засах
    async function handleEditModal(id, type) {
        setEditId(id)
        ...
        <Fragment>
            <Card>
                ...
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h5'>{t('Сургалтын төрөл')}</CardTitle>
                ...
                <div className='react-dataTable react-dataTable-selectable-rows' id='datatableLeftTwoRightOne'>
                    <DataTable
                        ...
                        columns={getColumns(currentPage, rowsPerPage, total_count, handleRequestDetail, handleEditModal)}
                ...
                { edit_modal && <EditModal is_edit={is_edit} isOpen={edit_modal} editId={edit_id} handleModal={handleEditModal} refreshDatas={getDatas} /> }
                { detailModalOpen && <Detail isOpen={detailModalOpen} editId={edit_id} handleModal={handleRequestDetail} datas={detailModalData} /> }
            </Card>
        </Fragment>
    )
};
