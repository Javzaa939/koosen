...
const RoleRegister = () => {
    ...
    async function getDatas() {
        const { success, data } = await allFetch(permissionStudentApi.get(rowsPerPage, currentPage, sortField, searchValue, student))
        if(success) {
    ...
    async function handleDelete(id) {
        if (id){
            const { success } = await fetchData(permissionStudentApi.delete(id))
            if(success) {
                getDatas()
            }
    ...
    async function getStudentOption() {
        const { success, data } = await fetchData(permissionStudentApi.getStudent())
        if(success) {
        ...
        <Fragment>
            <Card>
                ...
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>{t('Оюутны хичээл сонголтыг төлбөрөөс хамааралгүйгээр хийх эрхийн жагсаалт')}</CardTitle>
