...
const FileModal = ({ isOpen, handleModal, refreshData }) => {
    ...
    const fileApi = useApi().timetable.register
    ...
    async function onSubmit() {
        if (file) {
            ...
            const { success, error }  = await fetchData(fileApi.saveFile(formData))
            if (success) {
                handleModal()
                refreshData()
                setLoader(false)
    ...
    return (
        <Modal isOpen={isOpen} toggle={handleModal} className="modal-dialog-centered modal-sm">
            <ModalHeader toggle={handleModal}>{t('Хичээлийн хуваарь файлаар оруулах')}</ModalHeader>
            <ModalBody>
                <Label>{t('Файл')}</Label>
                <Input
                    type='file'
