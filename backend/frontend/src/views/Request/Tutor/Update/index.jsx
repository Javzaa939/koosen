...
const UpdateModal = ({ open, handleModal, is_view, editId, refreshDatas, datas }) => {
    ...
    async function getDatas() {
        if(editId) {
            const { success, data } = await fetchData(requestTutorApi.getOne(editId))
            if(success) {
                ...
                for(let key in data) {
                    ...
                    if(key === 'teacher') {
                            ...
                            setValue('full_name', data[key].full_name)
                        }
                    if(key === 'student') {
                        setValue('full_name', data[key].full_name)
                        setValue('code', data[key].code)
    ...
    async function onSubmit(cdata) {
        if(editId) {
            ...
            const { success, error } = await fetchData(requestTutorApi.put(cdata, editId))
            if(success) {
                refreshDatas()
                handleModal()
            }
                                    ...
                                    <table className='w-100'>
                                        <tbody>
                                            <tr className='border-bottom'>
                                                <td className='pe-1' style={{ width: '33%' }}>Хичээлийн код:</td>
                                                <td>
                                                    <span className='fw-bold'>{datas?.lesson?.code}</span>
                                                </td>
                                            </tr>
                                            <tr className='border-bottom'>
                                                <td className='pe-1' style={{ width: '33%' }}>Хичээлийн нэр:</td>
                                                <td>
                                                    <span className='fw-bold'>{datas?.lesson?.name}</span>
                                                </td>
                                            </tr>
                                            <tr className='border-bottom'>
                                            ...
                                            <tr className='border-bottom'>
                                                <td className='pe-1' style={{ width: '33%' }}>Багшийн код:</td>
                                                <td>
                                                    <span className='fw-bold'>{datas?.teacher?.code}</span>
                                                </td>
                                            </tr>
                                            <tr className='border-bottom'>
                                                <td className='pe-1' style={{ width: '33%' }}>Багшийн нэр:</td>
                                                <td>
                                                    <span className='fw-bold'>{datas?.teacher?.full_name}</span>
                                                </td>
                                            </tr>
                                            <tr className='border-bottom'>
                                                <td className='pe-1' style={{ width: '33%' }}>Дэлгэрэнгүй хүсэлт:</td>
                                                <td>
                                            ...
                                            render={({ field: { value, onChange} }) => {
                                                return (
                                                    <Select
                                                        ...
                                                        options={flag_option|| []}
                                                        value={flag_option.find((c) => c.id === value)}
