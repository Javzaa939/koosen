# Generated by Django 4.1 on 2024-09-16 10:47

from django.db import migrations
# Generated by Django 4.1 on 2024-09-03 08:07

from django.db import migrations
from django.db import transaction
from django.contrib.auth.hashers import make_password
from django.db import migrations

def custom_studentlogin_passwords(apps, schema_editor):
    """ Шууд нэмэгдсэн оюутнуудыг 3 курс login хэсэг рүү нэмэх """

    Student = apps.get_model('lms', 'Student')
    StudentLogin = apps.get_model('lms', 'StudentLogin')

    students = Student.objects.filter(updated_at__gte='2024-09-10').values('id', 'register_num', 'code')
    count = 0
    for student in list(students):
        count += 1
        password = student.get('register_num')[-8:]

        hashed_password = make_password(password)

        StudentLogin.objects.update_or_create(
            student_id=student.get('id'),
            defaults={
                'username': student.get('code'),
                'password': hashed_password
            }
        )
        print('Амжилттай үүсгэлээ', password, count)

    print(' Нийт оюутны мэдээлэл ',count)

class Migration(migrations.Migration):

    dependencies = [
        ('lms', '0253_challengequestioncount_challengesedevcount_and_more'),
    ]

    operations = [
        migrations.RunPython(custom_studentlogin_passwords),
    ]


class Migration(migrations.Migration):

    dependencies = [
        ('lms', '0259_remove_studentordersport_teacher_payment_and_more'),
    ]

    operations = [
        migrations.RunPython(custom_studentlogin_passwords),
    ]
